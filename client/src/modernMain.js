...
class GameScene extends Phaser.Scene {
  ...
  create(data) {
    ...
    this.enemies = [[1420, 920], [1570, 1370], [780, 1460]].map((pos, i) => { const enemy = this.makeUnit(pos[0], pos[1], 0xe45d5d, `RAIDER ${i + 1}`); enemy.hp = 55; enemy.maxHp = 55; enemy.lastShot = 0; return enemy; });
    this.keys = this.input.keyboard.addKeys('W,A,S,D,SPACE'); this.input.on('pointerdown', () => this.shootPlayer());
  ...
  buildWorld() {
    ...
    [[360, 370, 450, 70], [990, 500, 70, 360], [1450, 420, 450, 70], [350, 1100, 350, 70], [1240, 1530, 70, 370], [1580, 1680, 340, 70]].forEach(([x, y, w, h]) => { this.add.rectangle(x, y, w, h, 0x293f43).setOrigin(0).setStrokeStyle(2, 0x3f6261).setDepth(0); });
  ...
  setupHUD() {
    ...
    this.hud.add(label(this, 30, 674, 'WASD MOVE     MOUSE / SPACE FIRE', 12, '#91afba'));
  ...
  movePlayer(delta) { if (this.downed) return; const speed = 220 * delta / 1000; let dx = 0; let dy = 0; if (this.keys.A.isDown) dx--; if (this.keys.D.isDown) dx++; if (this.keys.W.isDown) dy--; if (this.keys.S.isDown) dy++; if (dx || dy) { const length = Math.hypot(dx, dy); this.player.x += dx / length * speed; this.player.y += dy / length * speed; } this.player.x = Phaser.Math.Clamp(this.player.x, 35, WORLD - 35); this.player.y = Phaser.Math.Clamp(this.player.y, 110, WORLD - 35); this.player.nameTag.setPosition(this.player.x - 34, this.player.y + 21); }
  updateAI(delta) { const distance = Phaser.Math.Distance.Between(this.ai.x, this.ai.y, this.player.x, this.player.y); const target = this.downed ? this.player : (this.enemies.find((enemy) => enemy.active) || this.player); const followX = this.downed ? target.x : this.player.x - this.sync.followDistance; const followY = this.downed ? target.y : this.player.y + 48; this.moveUnit(this.ai, { x: followX, y: followY }, this.downed ? 290 : (this.sync.aggressionLevel === 'aggressive' ? 250 : 155), delta); const enemy = this.enemies.find((item) => item.active); if (enemy && this.clock - this.ai.lastShot > this.sync.reactionDelayMs && Phaser.Math.Distance.Between(this.ai.x, this.ai.y, enemy.x, enemy.y) < 620) { this.ai.lastShot = this.clock; if (Math.random() < this.sync.aimAccuracy) this.hitEnemy(enemy, 18); } this.ai.nameTag.setPosition(this.ai.x - 45, this.ai.y + 21); }
  updateEnemies(delta) { this.enemies.forEach((enemy) => { if (!enemy.active) return; const target = this.downed ? this.ai : this.player; if (Phaser.Math.Distance.Between(enemy.x, enemy.y, target.x, target.y) < 700) { this.moveUnit(enemy, target, 42, delta); if (this.clock - enemy.lastShot > 1500) { enemy.lastShot = this.clock; if (!this.downed) { this.player.hp -= 8; if (this.player.hp <= 0) this.downPlayer(); } else { this.ai.hp -= 3; } } } enemy.nameTag.setPosition(enemy.x - 35, enemy.y + 20); }); }
  moveUnit(unit, target, speed, delta) { const angle = Phaser.Math.Angle.Between(unit.x, unit.y, target.x, target.y); unit.x += Math.cos(angle) * speed * delta / 1000; unit.y += Math.sin(angle) * speed * delta / 1000; }
  shootPlayer() { if (this.downed || this.clock - this.player.lastShot < 260) return; const target = this.enemies.find((enemy) => enemy.active && Phaser.Math.Distance.Between(this.player.x, this.player.y, enemy.x, enemy.y) < 850); if (!target) return; this.player.lastShot = this.clock; this.hitEnemy(target, 22); }
  hitEnemy(enemy, damage) { this.add.line(0, 0, this.player.x, this.player.y, enemy.x, enemy.y, 0xf4c95d).setLineWidth(2).setDepth(3); enemy.hp -= damage; if (enemy.hp <= 0) { enemy.setActive(false).setVisible(false); enemy.nameTag.setVisible(false); this.kills++; this.killText.setText(`KILLS  ${this.kills}`); this.say('enemy_killed'); if (!this.enemies.some((item) => item.active) && this.revived) this.finish(true); } }
