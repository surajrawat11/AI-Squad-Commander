...
class GameScene extends Phaser.Scene {
  ...
  setupHUD() {
    ...
    this.hud.add(label(this, 30, 674, 'WASD / ARROWS MOVE     MOUSE / SPACE FIRE', 12, '#91afba'));
  ...
  movePlayer(delta) { if (this.downed) return; const speed = 220 * delta / 1000; let dx = 0; let dy = 0; if (this.keys.A.isDown || this.keys.LEFT.isDown) dx--; if (this.keys.D.isDown || this.keys.RIGHT.isDown) dx++; if (this.keys.W.isDown || this.keys.UP.isDown) dy--; if (this.keys.S.isDown || this.keys.DOWN.isDown) dy++; if (dx || dy) { const length = Math.hypot(dx, dy); this.player.x += dx / length * speed; this.player.y += dy / length * speed; } this.player.x = Phaser.Math.Clamp(this.player.x, 35, WORLD - 35); this.player.y = Phaser.Math.Clamp(this.player.y, 110, WORLD - 35); this.player.nameTag.setPosition(this.player.x - 34, this.player.y + 21); }
