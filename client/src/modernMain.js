...
class GameScene extends Phaser.Scene {
  ...
  shootPlayer() { if (this.downed || this.clock - this.player.lastShot < 260) return; const pointer = this.input.activePointer; const targetX = pointer.worldX; const targetY = pointer.worldY; const target = this.enemies.filter((enemy) => enemy.active && Phaser.Math.Distance.Between(this.player.x, this.player.y, enemy.x, enemy.y) < 850).sort((a, b) => this.aimDistance(a, targetX, targetY) - this.aimDistance(b, targetX, targetY))[0]; if (!target || this.aimDistance(target, targetX, targetY) > 58) return; this.player.lastShot = this.clock; this.hitEnemy(target, 22, this.player); }
