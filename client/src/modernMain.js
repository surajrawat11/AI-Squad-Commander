...
class GameScene extends Phaser.Scene {
  ...
  makeUnit(x, y, color, name) { const shadow = this.add.ellipse(x, y + 10, 34, 12, 0x071016, 0.5).setDepth(2); const unit = this.add.rectangle(x, y, 26, 26, color).setStrokeStyle(2, 0xffffff, 0.7).setDepth(4); unit.shadow = shadow; unit.weapon = this.add.rectangle(x + 17, y, 28, 6, 0x17232a).setOrigin(0.15, 0.5).setDepth(5); unit.unitName = name; unit.nameTag = label(this, x - 34, y + 21, name, 10, color).setDepth(6); return unit; }
  updateOperatorVisual(unit, target) { unit.shadow.setPosition(unit.x, unit.y + 10); unit.weapon.setPosition(unit.x, unit.y); unit.weapon.rotation = Phaser.Math.Angle.Between(unit.x, unit.y, target.x, target.y); unit.nameTag.setPosition(unit.x - (unit.unitName === 'COMMANDER' ? 45 : 34), unit.y + 21); }
