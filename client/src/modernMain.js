...
class GameScene extends Phaser.Scene {
  ...
  makeUnit(x, y, color, name) { const shadow = this.add.ellipse(x, y + 11, 36, 13, 0x071016, 0.55).setDepth(2); const unit = this.add.circle(x, y, 15, color).setStrokeStyle(2, 0xf4f7e8, 0.85).setDepth(4); unit.shadow = shadow; unit.weapon = this.add.rectangle(x + 17, y, 30, 6, 0x17232a).setOrigin(0.12, 0.5).setDepth(5); unit.visor = this.add.rectangle(x + 3, y - 4, 12, 5, 0xd9f1e8, 0.85).setOrigin(0.5).setDepth(6); unit.unitName = name; unit.nameTag = label(this, x - 34, y + 24, name, 10, color).setDepth(7); return unit; }
  updateOperatorVisual(unit, target) { const angle = Phaser.Math.Angle.Between(unit.x, unit.y, target.x, target.y); unit.shadow.setPosition(unit.x, unit.y + 11); unit.weapon.setPosition(unit.x, unit.y); unit.weapon.rotation = angle; unit.visor.setPosition(unit.x + Math.cos(angle) * 5, unit.y + Math.sin(angle) * 5 - 4); unit.visor.rotation = angle; unit.nameTag.setPosition(unit.x - (unit.unitName === 'COMMANDER' ? 45 : 34), unit.y + 24); }
  ...
  updateEnemies(delta) { this.enemies.forEach((enemy) => { if (!enemy.active) return; const target = this.downed ? this.ai : this.player; if (Phaser.Math.Distance.Between(enemy.x, enemy.y, target.x, target.y) < 700) { this.moveUnit(enemy, target, 42, delta); if (this.clock - enemy.lastShot > 1500) { enemy.lastShot = this.clock; if (!this.downed) { this.player.hp -= 8; if (this.player.hp <= 0) this.downPlayer(); } else { this.ai.hp -= 3; } } this.updateOperatorVisual(enemy, target); enemy.hpBar.setPosition(enemy.x - 25, enemy.y - 23); enemy.hpBar.width = 50 * Math.max(enemy.hp, 0) / enemy.maxHp; } }); }
